    ...
    public partial class Branches : Form
    {
        ...
        private async void buttonBranchEmployees_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var branchEmployees = employeeList.Where(x => x.bID == int.Parse(textSearchID.Text)).ToList();


                dgByID.DataSource = branchEmployees;
            }
            catch (HttpRequestException x)
            {
                MessageBox.Show(x.Message);
        ...
        private void buttonPdfByID_Click(object sender, EventArgs e)
        {
            void ExportToPdf()
            {
                ...
                string path = $"C:\\PDF-HR\\PDF reports\\EmployeesByBranchID.pdf";
                ...
                var cell = new PdfPCell(new Phrase("Branch Employees"))
                {
                    Colspan = columnCount,
                    HorizontalAlignment = 1,
                    MinimumHeight = 30f
